  ...
  displayhash: async function () {
    ...
    if (window.location.hash === hashurl) {
      ...
      if (username) {
        ...
        let my_post = []
        await fetch("http://localhost:1337/posts?p_author.username="+username).then((res) => {
          return res.json()
        }).then((data) => {
          my_post = data
        })
        
        const target = document.getElementById("my_post")
        const template = Handlebars.compile(document.getElementById("my_post_view").textContent)
        const div = template({
          'picture': my_post
        })
        return target.innerHTML = div
    ...
    } else if (window.location.hash === "#!/all-posts") {
      const rec = Model.getRecentPosts()
      const target = document.getElementById("all_post")
      const template = Handlebars.compile(document.getElementById("all_post_view").textContent)
      console.log(document.getElementById("all_post_view"))
      const div = template({
        'picture': rec
      })
      return target.innerHTML = div
